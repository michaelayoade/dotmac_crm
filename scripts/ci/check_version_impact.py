#!/usr/bin/env python3
"""Validate that release-impacting PRs declare a version impact label.

Replaces the old manual "require a pyproject bump" gate with a label-driven one
(version:major|minor|patch|none). The bump itself is applied automatically after
merge by .github/workflows/version-bump-pr.yml, so feature PRs never edit the
version line (and never conflict on it).
"""
...
# Paths whose changes ship to users (and therefore need a SemVer decision).
RELEASE_PATTERNS = (
    "app/**",
    "alembic/**",
    "static/**",
...
# Paths that never ship (CI, docs, tests) — changing only these needs no label.
...
NONE_JUSTIFICATION_RE = re.compile(r"version impact:\s*none\s+because\s+\S", re.IGNORECASE)
...
def _deepen_base(base: str) -> None:
    ...
    if not remote or not branch:
        return
    subprocess.run(["git", "fetch", "--no-tags", remote, branch], capture_output=True, text=True)
