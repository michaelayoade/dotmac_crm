...
WORKTREE_DIR=/home/dotmac/projects/dotmac_crm/.worktrees/fix-deps-004
...
LOG_FILE=/home/dotmac/projects/dotmac_crm/.seabone/logs/fix-deps-004.log
TASK_ID=fix-deps-004
DESCRIPTION=Replace\ unmaintained\ passlib\ 1.7.4\ with\ direct\ bcrypt\ usage.\ passlib\ has\ had\ no\ releases\ since\ 2020\ and\ emits\ deprecation\ warnings\ with\ modern\ bcrypt\>=4.0.\ Steps:\ 1\)\ Run:\ poetry\ remove\ passlib\ \&\&\ poetry\ add\ \'bcrypt\>=4.0\'.\ 2\)\ In\ app/services/auth.py\ replace\ passlib\ imports:\ remove\ \'from\ passlib.context\ import\ CryptContext\'\ and\ \'pwd_context\ =\ CryptContext\(schemes=\[\"bcrypt\"\]\)\'.\ 3\)\ Replace\ pwd_context.hash\(password\)\ with:\ import\ bcrypt\;\ bcrypt.hashpw\(password.encode\(\)\,\ bcrypt.gensalt\(\)\).decode\(\).\ 4\)\ Replace\ pwd_context.verify\(plain\,\ hashed\)\ with:\ bcrypt.checkpw\(plain.encode\(\)\,\ hashed.encode\(\)\).\ 5\)\ Search\ for\ other\ passlib\ usage:\ grep\ -r\ \'passlib\'\ app/\ and\ fix\ all\ occurrences.\ 6\)\ Existing\ bcrypt\ hashes\ in\ the\ database\ remain\ compatible.\ 7\)\ Run:\ pytest\ tests/\ -x\ -q\ to\ verify\ login/password\ flows\ still\ work.
BRANCH=agent/fix-deps-004
ENGINE=codex
MODEL=gpt-5.3-codex
