...
ApiClient _client(FakeHttpAdapter adapter, TokenStore store, {void Function()? onExpired}) {
...
void main() {
  ...
  test('staff login stores tokens and mode', () async {
    adapter.on('POST', '/api/v1/auth/login', (_) => (200, {
          'access_token': freshToken,
          'refresh_token': 'refresh-1',
        }));

    final result = await repo.login(username: 'tech@dotmac.io', password: 'pw', mode: LoginMode.staff);
  ...
  test('vendor login uses vendor path and captures vendor context', () async {
    adapter.on('POST', '/api/v1/vendor/auth/login', (_) => (200, {
          'access_token': freshToken,
          'refresh_token': 'refresh-v',
          'vendor_id': 'vendor-9',
        }));

    final result = await repo.login(username: 'crew@vendor.io', password: 'pw', mode: LoginMode.vendor);
  ...
  test('mfa challenge then verify', () async {
    adapter.on('POST', '/api/v1/auth/login', (_) => (200, {'mfa_required': true, 'mfa_token': 'mfa-1'}));
    adapter.on('POST', '/api/v1/auth/mfa/verify', (_) => (200, {
          'access_token': freshToken,
          'refresh_token': 'refresh-2',
        }));

    final challenge = await repo.login(username: 'tech@dotmac.io', password: 'pw', mode: LoginMode.staff);
  ...
  test('bad credentials surface server detail', () async {
    adapter.on('POST', '/api/v1/auth/login', (_) => (401, {'detail': 'Invalid credentials'}));
    final result = await repo.login(username: 'x', password: 'y', mode: LoginMode.staff);
  ...
  test('401 triggers refresh and retries once with the new token', () async {
    ...
    final revoked = fakeJwt(expiry: DateTime.now().toUtc().add(const Duration(minutes: 10)));
    await store.save(accessToken: revoked, refreshToken: 'refresh-old', loginMode: LoginMode.staff);
    ...
    adapter.on('POST', '/api/v1/auth/refresh', (_) => (200, {
          'access_token': freshToken,
  ...
  test('proactive refresh fires before expiry without a 401', () async {
    ...
    adapter.on('POST', '/api/v1/auth/refresh', (_) => (200, {'access_token': freshToken}));
    adapter.on('GET', '/api/v1/field/me', (options) {
      final auth = options.headers['Authorization'] as String?;
      return auth == 'Bearer $freshToken' ? (200, {'ok': true}) : (401, {'detail': 'stale'});
    });
  ...
  test('refresh failure signals session expiry', () async {
    ...
    adapter.on('POST', '/api/v1/auth/refresh', (_) => (401, {'detail': 'Account disabled'}));
  ...
  test('config gate blocks below min version and merges flags', () async {
    adapter.on('GET', '/api/v1/field/config', (_) => (200, {
