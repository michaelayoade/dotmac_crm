...
void main() {
  ...
  ProviderContainer makeContainer({required bool authenticated}) {
    ...
    store.save(accessToken: freshToken, refreshToken: 'r', loginMode: LoginMode.staff);

    return ProviderContainer(overrides: [
      apiClientProvider.overrideWithValue(client),
      pushSourceProvider.overrideWithValue(push),
      authControllerProvider.overrideWith(authenticated ? _AuthedController.new : _UnauthedController.new),
    ]);
  }
  ...
  test('registers the device token when authenticated', () async {
    ...
    final registrar = PushRegistrar(_RefAdapter(container), onDeepLink: deepLinks.add);
  ...
  test('token rotation re-registers', () async {
    ...
    final registrar = PushRegistrar(_RefAdapter(container), onDeepLink: deepLinks.add)..start();
    push.rotateToken('fcm-token-2');
    await Future<void>.delayed(const Duration(milliseconds: 20));
    expect(tokens, ['fcm-token-2']);
  ...
  test('notification tap deep-links to the job', () async {
    ...
    push.emit(const PushMessage(fromTap: false, data: {'type': 'work_order_assigned', 'work_order_id': 'x'}));
    push.emit(const PushMessage(fromTap: true, data: {'type': 'unknown'}));
    await Future<void>.delayed(const Duration(milliseconds: 20));

    expect(deepLinks, ['/jobs/wo-9']);
    await registrar.dispose();
  });

  test('routeForMessage mapping', () {
    expect(routeForMessage({'type': 'work_order_assigned', 'work_order_id': 'a'}), '/jobs/a');
...
class _RefAdapter implements Ref {
  ...
  ProviderSubscription<T> listen<T>(
    ...
    bool fireImmediately = false,
      ...
      providerContainer.listen(provider, listener, onError: onError, fireImmediately: fireImmediately);
