...
def test_base_handler_receive_post_commit():
    ...
    with patch(
        "app.services.crm.inbox.handlers.base.create_message_and_touch_conversation"
    ) as mock_create, patch(
        "app.services.crm.inbox.handlers.base.post_process_inbound_message"
    ) as mock_post, patch(
        "app.services.crm.inbox.handlers.base.emit_event"
    ) as mock_emit, patch(
        "app.services.crm.inbox.handlers.base.event.listen"
    ) as mock_listen:
...
def test_whatsapp_handler_process_success():
    ...
    with patch(
        "app.services.crm.inbox.handlers.whatsapp._resolve_integration_target"
    ) as mock_target, patch(
        "app.services.crm.inbox.handlers.whatsapp._resolve_connector_config"
    ) as mock_config, patch(
        "app.services.crm.inbox.handlers.whatsapp._is_self_whatsapp_message"
    ) as mock_self, patch(
        "app.services.crm.inbox.handlers.whatsapp._resolve_person_for_inbound"
    ) as mock_person, patch(
        "app.services.crm.inbox.handlers.whatsapp._find_duplicate_inbound_message"
    ) as mock_dedupe, patch(
        "app.services.crm.inbox.handlers.whatsapp.conversation_service"
    ) as mock_conv:
        ...
        mock_conv.Conversations.create.return_value = SimpleNamespace(
            id="conv-1", is_active=True
        )
...
def test_email_handler_process_success():
    ...
    with patch(
        "app.services.crm.inbox.handlers.email._resolve_integration_target"
    ) as mock_target, patch(
        "app.services.crm.inbox.handlers.email._resolve_connector_config"
    ) as mock_config, patch(
        "app.services.crm.inbox.handlers.email._is_self_email_message"
    ) as mock_self, patch(
        "app.services.crm.inbox.handlers.email._resolve_person_for_inbound"
    ) as mock_person, patch(
        "app.services.crm.inbox.handlers.email._find_duplicate_inbound_message"
    ) as mock_dedupe, patch(
        "app.services.crm.inbox.handlers.email._normalize_external_id"
    ) as mock_normalize, patch(
        "app.services.crm.inbox.handlers.email._resolve_conversation_from_email_metadata"
    ) as mock_resolve, patch(
        "app.services.crm.inbox.handlers.email.conversation_service"
    ) as mock_conv:
        mock_target.return_value = SimpleNamespace(id="target-1")
        mock_config.return_value = None
        mock_self.return_value = False
