...
class ConversationPage(BasePage):
    ...
    def expect_loaded(self) -> None:
        ...
        expect(
            self.page.locator("[data-testid='message-thread']")
            .or_(self.page.locator(".message-thread").or_(self.page.get_by_role("textbox")))
            .first
        ).to_be_visible()

    def expect_messages_visible(self) -> None:
        """Assert messages are visible."""
        expect(self.page.locator("[data-testid='message']").or_(self.page.locator(".message")).first).to_be_visible()

    def get_message_count(self) -> int:
        """Get count of messages in the thread."""
        messages = self.page.locator("[data-testid='message']").or_(self.page.locator(".message"))
        return messages.count()

    def send_reply(self, message: str) -> None:
        """Send a reply message."""
        self.page.get_by_placeholder("message").or_(self.page.get_by_role("textbox")).first.fill(message)
    ...
    def expect_contact_details_visible(self) -> None:
        ...
        expect(
            self.page.locator("[data-testid='contact-details']")
            .or_(self.page.get_by_text("Contact", exact=False))
            .first
        ).to_be_visible()

    def get_contact_name(self) -> str:
        """Get contact name from details."""
        name_element = self.page.locator("[data-testid='contact-name']").or_(self.page.get_by_role("heading").first)
        return name_element.text_content() or ""

    def get_contact_email(self) -> str:
        """Get contact email from details."""
        email_element = self.page.locator("[data-testid='contact-email']").or_(self.page.get_by_text("@")).first
        return email_element.text_content() or ""

    def add_note(self, note: str) -> None:
        """Add a note to the contact."""
        self.page.get_by_role("button", name="Note").first.click()
        self.page.get_by_placeholder("note").or_(self.page.get_by_role("textbox")).first.fill(note)
    ...
    def mark_as_resolved(self) -> None:
        ...
        self.page.get_by_role("button", name="Resolve").or_(self.page.get_by_role("button", name="Close")).first.click()
