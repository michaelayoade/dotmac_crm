...
class VendorAsBuiltPage(BasePage):
    ...
    def expect_loaded(self) -> None:
        ...
        expect(
            self.page.get_by_role("heading", name="As-Built", exact=True)
            .or_(self.page.get_by_role("heading", name="Built", exact=True))
            .first
        ).to_be_visible()

    def fill_completion_date(self, date: str) -> None:
        """Fill completion date."""
        self.page.get_by_label("Completion").or_(self.page.get_by_label("Date")).first.fill(date)
    ...
    def upload_photo(self, file_path: str) -> None:
        ...
        self.page.get_by_label("Photo").or_(self.page.locator("input[type='file']")).first.set_input_files(file_path)

    def upload_document(self, file_path: str) -> None:
        """Upload supporting document."""
        self.page.get_by_label("Document").or_(self.page.locator("input[type='file']")).first.set_input_files(file_path)

    def fill_fiber_length(self, length: str) -> None:
        """Fill fiber length installed."""
        self.page.get_by_label("Fiber Length").or_(self.page.get_by_label("Length")).first.fill(length)

    def fill_ont_serial(self, serial: str) -> None:
        """Fill ONT serial number."""
        self.page.get_by_label("ONT Serial").or_(self.page.get_by_label("Serial")).first.fill(serial)
