...
def test_workqueue_defaults_to_me_even_with_broader_permissions(db_session, set_setting):
    set_setting("workqueue.enabled", True)
    with _build_app(
        db_session,
        permissions=["workqueue:view", "workqueue:audience:team", "workqueue:audience:org"],
    ) as app:
        resp = _run_async(_aget(app, "/agent/workqueue"))
    assert resp.status_code == 200, resp.text
