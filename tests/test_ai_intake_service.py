...
def _make_person(db_session):
    person = Person(
        email=f"ai-intake-{uuid.uuid4().hex[:8]}@example.com",
        first_name="AI",
        last_name="Test",
...
def test_process_pending_intake_profile_reply_updates_person_and_finalizes_handoff(db_session, monkeypatch):
    ...
    assert result.handled is True
    assert result.resolved is True
    assert person.date_of_birth == date(1994, 5, 20)
    assert person.gender == Gender.female
    assert resync_calls == [(str(person.id), {"date_of_birth", "gender"})]
    assert conversation.status == ConversationStatus.open
    assert assignment is not None
    assert assignment.team_id == team.id
    assert assignment.agent_id == agent.id
...
def test_process_pending_intake_profile_reply_updates_selfcare_managed_person(db_session, monkeypatch):
    person = _make_person(db_session)
    person.metadata_ = {"selfcare_id": "sc-1"}
    person.date_of_birth = None
    person.gender = Gender.unknown
    ...
    assert state["profile_collection_completed"] is True
    assert set(state["profile_collection_updated_fields"]) == {"date_of_birth", "gender"}
    assert state.get("profile_collection_skipped") is not True
...
@pytest.mark.parametrize(
    "body",
    [
        "Date of birth: 1979-08-31, gender: male",
        "Date of birth: 1979-08-31; gender: male",
    ],
)
def test_parse_profile_reply_accepts_inline_labeled_fields(body):
    parsed, error = _parse_profile_reply(body, ["date_of_birth", "gender"])

    assert error is None
    assert parsed["date_of_birth"] == date(1979, 8, 31)
...
def test_process_pending_intake_profile_reply_allows_two_retries_then_records_failure_marker(db_session, monkeypatch):
    ...
    conversation.metadata_ = {
        AI_INTAKE_METADATA_KEY: {
            ...
            "profile_collection": {
                ...
                "missing_standard_fields": ["date_of_birth"],
                "attempt_count": 0,
                "requested_at": now.isoformat(),
                "department": "support",
            },
    ...
    db_session.refresh(conversation)
    state = conversation.metadata_[AI_INTAKE_METADATA_KEY]
    assert first_result.followup_sent is True
    assert conversation.status == ConversationStatus.pending
    assert state["status"] == "awaiting_profile"
    ...
    assert state["profile_collection_failed_fields"] == ["date_of_birth"]


def test_process_pending_intake_missing_person_record_logs_invariant_and_handoffs(db_session, monkeypatch, caplog):
    person = _make_person(db_session)
    conversation = _make_conversation(db_session, person)
    conversation.person_id = uuid.uuid4()
    db_session.commit()
    widget_id = str(uuid.uuid4())
    message = _make_message(db_session, conversation, metadata={"widget_config_id": widget_id})
    team = CrmTeam(name="Support", is_active=True)
    ...
    with caplog.at_level("ERROR", logger="app.services.crm.ai_intake"):
        result = process_pending_intake(
            db_session,
            conversation=conversation,
            message=message,
    ...
    assert result.resolved is True
    assert conversation.status == ConversationStatus.open
    assert conversation.metadata_[AI_INTAKE_METADATA_KEY]["profile_collection_skipped"] is True
    assert "ai_intake_profile_person_invariant_violation" in caplog.text


def test_escalate_expired_pending_intakes_skips_profile_collection(db_session, monkeypatch):
    person = _make_person(db_session)
    conversation = _make_conversation(db_session, person)
    widget_id = str(uuid.uuid4())
    _make_config(db_session, scope_key=f"widget:{widget_id}")
    ...
    result = escalate_expired_pending_intakes(db_session)

    db_session.refresh(conversation)
    assert result["escalated"] == 0
    assert result["skipped"] == 1
    assert conversation.status == ConversationStatus.pending
    assert conversation.metadata_[AI_INTAKE_METADATA_KEY]["status"] == "awaiting_profile"
...
def test_reassign_stale_ai_handoff_preserves_overdue_manual_assignment(db_session):
    manager = _make_person(db_session)
    person = _make_person(db_session)
    conversation = _make_conversation(db_session, person)
    team = CrmTeam(name="Manual Handoff Support", is_active=True)
    db_session.add(team)
    db_session.commit()
    agent = _make_agent(db_session, team, label="ManualFirst", status=AgentPresenceStatus.online)
    assigned_at = datetime.now(UTC) - timedelta(minutes=20)
    conversation.status = ConversationStatus.open
    conversation.metadata_ = {
        AI_INTAKE_METADATA_KEY: {
...
def test_reassign_stale_ai_handoff_preserves_overdue_cross_team_manual_assignment(db_session):
    manager = _make_person(db_session)
    person = _make_person(db_session)
    conversation = _make_conversation(db_session, person)
    routed_team = CrmTeam(name="Routed Handoff Team", is_active=True)
    assigned_team = CrmTeam(name="Cross Team Handoff", is_active=True)
    db_session.add_all([routed_team, assigned_team])
    db_session.commit()
    agent = _make_agent(db_session, assigned_team, label="CrossTeam", status=AgentPresenceStatus.online)
    assigned_at = datetime.now(UTC) - timedelta(minutes=20)
    ai_metadata = {
        "status": "escalated",
        "escalated_at": assigned_at.isoformat(),
        "routing_selected_team_id": str(routed_team.id),
        "routing_assigned_team_id": str(assigned_team.id),
        "routing_assigned_agent_id": str(agent.id),
        "handoff_state": "assigned",
    }
    conversation.status = ConversationStatus.open
    conversation.metadata_ = {AI_INTAKE_METADATA_KEY: ai_metadata.copy()}
    db_session.add(
        ConversationAssignment(
            conversation_id=conversation.id,
    ...
    assert result["processed"] == 0
    assert assignment.agent_id == agent.id
    assert assignment.team_id == assigned_team.id
    assert assignment.assigned_by_id == manager.id
    assert conversation.metadata_[AI_INTAKE_METADATA_KEY] == ai_metadata


def test_reassign_stale_ai_handoff_does_not_overwrite_manual_reassignment_during_selection(db_session, monkeypatch):
    manager = _make_person(db_session)
    person = _make_person(db_session)
    conversation = _make_conversation(db_session, person)
    team = CrmTeam(name="Handoff Race Team", is_active=True)
    db_session.add(team)
    db_session.commit()
    first_agent = _make_agent(db_session, team, label="RaceFirst", status=AgentPresenceStatus.online)
    second_agent = _make_agent(db_session, team, label="RaceSecond", status=AgentPresenceStatus.online)
    assigned_at = datetime.now(UTC) - timedelta(minutes=20)
    conversation.status = ConversationStatus.open
    conversation.metadata_ = {
        AI_INTAKE_METADATA_KEY: {
    ...
    def manually_reassign(*args, **kwargs):
    ...
    monkeypatch.setattr("app.services.crm.ai_intake._select_reassignment_agent", manually_reassign)
    ...
    assert assignment.agent_id == first_agent.id
    assert assignment.team_id == team.id
    assert assignment.assigned_by_id == manager.id
...
def test_background_ncc_profile_capture_skips_selfcare_managed_subscriber(db_session, monkeypatch):
    person, conversation = _make_missing_profile_conversation(db_session)
    subscriber = Subscriber(person_id=person.id, external_system="selfcare", external_id="sc-1")
    db_session.add(subscriber)
    message = _make_message(
        db_session,
        conversation,
    ...
    assert person.gender == Gender.unknown
    state = conversation.metadata_["ncc_profile_background_capture"]
    assert state["last_status"] == "skipped_selfcare_managed_profile"
    assert state.get("last_scanned_message_id") != str(message.id)
