...
class TestMaterialRequestStatusTransitions:
    ...
    def test_approve_validates_selected_serial_by_search_when_erp_has_more(
        self,
        db_session,
        person,
        ticket,
        ...
        mock_sync_service = MagicMock()
        mock_sync_service.client.get_inventory_items.return_value = [{"item_code": inventory_item.sku}]
        mock_sync_service.client.list_available_serials.side_effect = [
            {
                "track_serial_numbers": True,
                "serials": [{"serial_number": "ONT-001"}],
                "has_more": True,
        ...
        with patch("app.tasks.integrations.sync_material_request_to_erp.delay"):
            material_requests.approve(
                ...
                str(person.id),
                source_location_id=str(inventory_location.id),
                serial_numbers_by_item={str(line.id): ["ONT-999"]},
            )

        mock_sync_service.client.list_available_serials.assert_any_call(
            item_code=inventory_item.sku,
            warehouse_code=inventory_location.code,
            limit=20,
            search="ONT-999",
        )
