...
def _get_batch_size(db=None) -> int:
    ...
    return _coerce_int(size, _DEFAULT_BATCH_SIZE)
...
def _get_hot_retention_hours(db=None) -> int:
    ...
    return _coerce_int(hours, _DEFAULT_HOT_RETENTION_HOURS)
...
def _get_redis_stream_max_length(db=None) -> int:
    ...
    return _coerce_int(length, _DEFAULT_REDIS_STREAM_MAX_LENGTH)
...
def _get_redis_read_timeout_ms(db=None) -> int:
    ...
    return _coerce_int(timeout, _DEFAULT_REDIS_READ_TIMEOUT_MS)
