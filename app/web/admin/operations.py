...
@router.get(
    "/work-orders",
    response_class=HTMLResponse,
    dependencies=[Depends(require_any_permission("operations:work_order:read", "support:ticket:read"))],
)
...
def work_order_new(
    ...
    if ticket_id:
        ...
        if not linked_ticket:
            try:
                linked_ticket = db.get(Ticket, coerce_uuid(ticket_id))
            except (ValueError, AttributeError):
                linked_ticket = None
        if linked_ticket:
            ticket_ref = linked_ticket.number or str(linked_ticket.id)
            ticket_priority = linked_ticket.priority.value if linked_ticket.priority else "normal"
            form_defaults = {
                "title": f"Field work for ticket #{ticket_ref}",
                "description": linked_ticket.description or linked_ticket.title or "",
                "status": "draft",
                "priority": ticket_priority if ticket_priority in {p.value for p in WorkOrderPriority} else "normal",
                "work_type": "repair",
...
async def work_order_status_update(
    request: Request,
    order_id: UUID,
    db: Session = Depends(get_db),
):
    """Quick inline status update for a work order."""
    form = await request.form()
    status_raw = form.get("status")
    status_value = status_raw.strip() if isinstance(status_raw, str) else ""
    redirect_url = request.headers.get("referer") or "/admin/operations/work-orders"
    ...
    try:
        workforce_service.work_orders.update(
            ...
            str(order_id),
            WorkOrderUpdate(
                status=WorkOrderStatus(status_value) if status_value else None,
            ),
        )
        if request.headers.get("HX-Request"):
            return HTMLResponse(
...
@router.post(
    ...
    response_class=HTMLResponse,
    dependencies=[Depends(require_any_permission("operations:work_order:dispatch", "operations:work_order:update"))],
)
async def work_order_assign(
    ...
    """Quick inline technician assignment for a work order."""
    form = await request.form()
    assigned_raw = form.get("assigned_to_person_id")
    assigned_value = assigned_raw.strip() if isinstance(assigned_raw, str) else ""
    redirect_url = request.headers.get("referer") or "/admin/operations/work-orders"
    ...
    try:
        workforce_service.work_orders.update(
            ...
            str(order_id),
            WorkOrderUpdate(
                assigned_to_person_id=UUID(assigned_value) if assigned_value else None,
            ),
        )
        if request.headers.get("HX-Request"):
            return HTMLResponse(
