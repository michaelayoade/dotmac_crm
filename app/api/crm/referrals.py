"""Referral program API: admin management + public capture.

Admin routes are gated by the existing `crm:lead:*` permissions (referrals are
part of the sales/lead funnel) so no new RBAC seeding is required; dedicated
`crm:referral:*` permissions can be split out later. The capture endpoint is
public (a prospect using a shared referral link) and lives on a no-auth router.
"""
...
from app.schemas.crm.referral import (
...
def reject_referral(referral_id: str, payload: ReferralRejectRequest, db: Session = Depends(get_db)):
    return referrals_service.reject(db, referral_id, payload.reason)


@router.post(
    "/people/{person_id}/referral-code",
    response_model=ReferralCodeRead,
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(require_permission("crm:lead:write"))],
)
def ensure_referral_code(person_id: str, db: Session = Depends(get_db)):
    """Get or mint the active referral code for a referrer (an active subscriber)."""
    return referrals_service.ensure_code(db, person_id)
