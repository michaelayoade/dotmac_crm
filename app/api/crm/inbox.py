...
def send_message(payload: InboxSendRequest, db: Session = Depends(get_db), _user=Depends(get_current_user)):
    try:
...
def create_template(payload: MessageTemplateCreate, db: Session = Depends(get_db), _user=Depends(get_current_user)):
    return message_templates.create(db, payload)


@router.get("/templates/{template_id}", response_model=MessageTemplateRead)
def get_template(template_id: str, db: Session = Depends(get_db), _user=Depends(get_current_user)):
    return message_templates.get(db, template_id)


@router.patch("/templates/{template_id}", response_model=MessageTemplateRead)
def update_template(
    template_id: str, payload: MessageTemplateUpdate, db: Session = Depends(get_db), _user=Depends(get_current_user)
):
    return message_templates.update(db, template_id, payload)


@router.delete("/templates/{template_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_template(template_id: str, db: Session = Depends(get_db), _user=Depends(get_current_user)):
...
def create_email_connector(
    payload: EmailConnectorCreate, db: Session = Depends(get_db), _user=Depends(get_current_user)
):
...
def create_email_polling_job(
    payload: EmailPollingJobRequest, db: Session = Depends(get_db), _user=Depends(get_current_user)
):
