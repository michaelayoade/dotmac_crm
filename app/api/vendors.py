...
# Permission gates. These privileged vendor-admin operations were previously
...
def create_vendor(payload: VendorCreate, db: Session = Depends(get_db), auth: dict = Depends(VENDOR_WRITE)):
    return vendor_service.vendors.create(db, payload)


@router.get("/{vendor_id}", response_model=VendorRead)
def get_vendor(vendor_id: str, db: Session = Depends(get_db), auth: dict = Depends(VENDOR_READ)):
...
def update_vendor(
    vendor_id: str, payload: VendorUpdate, db: Session = Depends(get_db), auth: dict = Depends(VENDOR_WRITE)
):
    return vendor_service.vendors.update(db, vendor_id, payload)


@router.delete("/{vendor_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_vendor(vendor_id: str, db: Session = Depends(get_db), auth: dict = Depends(VENDOR_WRITE)):
...
def create_installation_project(
    payload: InstallationProjectCreate, db: Session = Depends(get_db), auth: dict = Depends(PROJECT_WRITE)
):
...
def assign_vendor(project_id: str, vendor_id: str, db: Session = Depends(get_db), auth: dict = Depends(PROJECT_WRITE)):
...
def approve_quote(
    ...
    return vendor_service.project_quotes.approve(
        db,
        quote_id,
        reviewer_person_id=auth["person_id"],
...
def compare_as_built(as_built_id: str, db: Session = Depends(get_db), auth: dict = Depends(PROJECT_READ)):
