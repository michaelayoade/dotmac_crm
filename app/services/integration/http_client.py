"""Shared HTTP transport for cross-app integration clients (audit keystone A2).

Every integration edge (crm→sub, sub→crm, crm→erp, erp→sub) re-implements the
same transport plumbing: a bounded retry loop with backoff, Retry-After
honouring, an optional reachability circuit breaker, auth-header injection, and
an idempotency-key header. This centralises that loop so each edge only declares
its *policy* — which exceptions retry, how a response maps to a parsed body or a
...
First consumer: the DotMac ERP client. The selfcare (crm→sub) client — currently
on ``requests`` — adopts this next once migrated to httpx.
"""
...
class IntegrationHttpClient:
    ...
    def __init__(
        ...
        *,
        ...
        transport_exhausted_factory: Callable[[BaseException, int], BaseException] | None = None,
        loop_exhausted_factory: Callable[[BaseException, int], BaseException] | None = None,
        unexpected_error_factory: Callable[[BaseException], BaseException] | None = None,
        ...
        *,
        ...
        for attempt in range(self._max_attempts):
            try:
                ...
                if self._rate_limit_exc is not None and isinstance(exc, self._rate_limit_exc):
                ...
                if self._retryable_excs and isinstance(exc, self._retryable_excs):
                    if attempt < retries:
                        ...
                        continue
                    raise
                if self._non_retryable_excs and isinstance(exc, self._non_retryable_excs):
                    raise
    ...
    def _circuit_open_error(self, path: str) -> BaseException:
        if self._loop_exhausted_factory is not None:
            return self._loop_exhausted_factory(RuntimeError(f"circuit open for {path}"), 0)
        return RuntimeError(f"integration temporarily unavailable (circuit open) for {path}")
